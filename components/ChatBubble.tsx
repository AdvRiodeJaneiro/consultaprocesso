    ...
    <div className={`flex w-full mb-4 ${isUser ? 'justify-end' : 'justify-start'}`}>
      <div
        className={`max-w-[85%] md:max-w-[75%] rounded-2xl px-5 py-4 shadow-sm ${
          isUser
            ? 'bg-[#dfa968] text-slate-900 rounded-br-none font-medium'
          ...
          <div className="prose prose-sm max-w-none prose-invert">
             <div className="whitespace-pre-wrap leading-relaxed">
                {message.content.split('\n').map((line, i) => {
                    ...
                    if (line.startsWith('### ')) return <h3 key={i} className="text-md font-bold mt-3 mb-2 text-[#dfa968] uppercase tracking-wide">{line.replace('### ', '')}</h3>;
                    if (line.startsWith('- ')) return <li key={i} className="ml-4">{line.replace('- ', '')}</li>;
                    
                    const matchLink = line.match(/\[(.*?)\]\((.*?)\)/);
                    if (matchLink) {
                        // For the specific case where the AI returns text mixed with a link, 
                        // we need a slightly better split.
                                    ...
                                    if (linkMatch) {
                                        return (
                                            <a 
                                                ...
                                                className="text-[#dfa968] underline hover:text-white transition-colors"
                                            >
                                                {linkMatch[1]}
                                            </a>
                                        );
                                    }
                                    // Handle bold inside non-link parts
                    ...
                    // Default rendering for lines without links
                    return (
                        <p key={i} className="mb-2">
